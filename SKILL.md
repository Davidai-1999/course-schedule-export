...
description: Guided extraction, verification, and calendar export for university or school timetables. Use when an agent needs to convert a timetable from xls/xlsx, screenshots, PDF, OCR text, or pasted schedule text into a verified .ics calendar file for Apple Calendar, Google Calendar, or similar apps. Especially use when week-1 alignment, school-specific period times, timezone, odd/even weeks, merged class periods, supplemental screenshots, or ambiguous week notation must be checked before export.
---
